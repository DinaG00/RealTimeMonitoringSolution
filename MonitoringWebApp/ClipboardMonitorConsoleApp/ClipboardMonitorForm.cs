        ...
        private async void SaveClipboardToDatabase(string content)
        {
            try
            {
                using (HttpClient client = new HttpClient())
                {
                    ...
                    {
                        content = content // Changed from 'text' to 'content' to match server expectations
                    });

                    var contentData = new StringContent(json, Encoding.UTF8, "application/json");
                    var response = await client.PostAsync("http://localhost:5001/logs/clipboard", contentData); // Updated endpoint URL
                    ...
                    {
                        Console.WriteLine("Successfully sent clipboard content to server");
                    }
                }
            }
            catch (Exception ex)
